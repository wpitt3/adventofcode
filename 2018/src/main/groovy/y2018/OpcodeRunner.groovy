...
class OpcodeRunner {
    static List<List<Integer>> readInstructions() {
        List<String> lines = new File("../../resources/16.txt").readLines()
        lines.take(3188).collate(4).collect(({
            it.take(3).collect { line ->
                line.replaceAll('[^0-9 ]', '')
                        .trim()
                        .split(' ')
                        .collect { Integer.parseInt(it) }
            }
        } as Closure<List<Integer>>))
    }

    static Map<String, Closure> generateFunctions() {
    ...
    static Map<String, Closure> generateLongFunctions() {
        Closure byR = { long rIndex, List registers -> registers[rIndex] }
        Closure byV = { long rIndex, List registers -> rIndex }

        Closure add = { long a, long b -> a + b }
        Closure multi = { long a, long b -> a * b }
        Closure and = { long a, long b -> a & b }
        Closure or = { long a, long b -> a | b }
        Closure assign = { long a, long b -> a }
        Closure gThan = { long a, long b -> a>b?1:0 }
        Closure equals = { long a, long b -> a==b?1:0 }

        Closure buildIt = { Closure method, Closure aBy, Closure bBy ->
            return { long a, long b, long c, List registers ->
                List x = registers.clone()
                x[c] = method( aBy(a,registers), bBy(b,registers) )
                return x
            }
